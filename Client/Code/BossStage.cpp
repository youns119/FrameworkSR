...
HRESULT CBossStage::Ready_Layer_Player(const _tchar* _pLayerTag)
{
	...
	pGameObject = CPlayer::Create(m_pGraphicDev, _vec3{ 19.5f, 1.f, 1.f }, TRUE);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
...
HRESULT CBossStage::Ready_Layer_Effect(const _tchar* _pLayerTag)
{
	...
	pGameObject = CEffectPool::Create(m_pGraphicDev, (CGameObject * (*)(LPDIRECT3DDEVICE9))CEffectShockWave::Create);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"EffectPool_ShockWave", pGameObject), E_FAIL);

	pGameObject = CEffectPool::Create(m_pGraphicDev, (CGameObject * (*)(LPDIRECT3DDEVICE9))CEffectBigExplosion::Create);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"EffectPool_BigExplosion", pGameObject), E_FAIL);
	...
	pGameObject = CEffectBossRobotDie::Create(m_pGraphicDev);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"EffectBossRobotDie", pGameObject), E_FAIL);
